...
class BaseOwfRestController {
    ...
    protected getJsonResult(result, targetProperty, params) {
        ...
        if (params.isExtAjaxFormat != null && params.isExtAjaxFormat == 'true') {
            return [success:result.success, data: ( (gotFromTargetProperty) ? gotFromTargetProperty.toServiceModel() : result)] as JSON
        }
        else {
            return (gotFromTargetProperty) ? gotFromTargetProperty.toServiceModel() as JSON : result as JSON
        }
    }
