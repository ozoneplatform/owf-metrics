...
//      s longOpt: 'webapp_directory', 'Specifiy location of the directory of where the OWF webapp was extracted'
    ...
    if (options.h) {
      ...
      println " 3) Change directory to the WEB-INF/tools (the next command must be executed from this directory)"
      println " 4) Execute java -jar createWebBundles.jar "
      println " 5) Repackage the temporary directory as a new OWF war file"
      println " 6) Deploy the war to the web server"
      println " 7) Inform any existing users they need to clear their browser cache"
      cli.usage()
      return
    }
